#!/usr/bin/env node
/**
 * Prepare Ffw2025 bundled assets (live 2025/index.aspx surface only):
 * - Convert stills → WebP (q78)
 * - Copy gallery GIF as-is
 * - Copy Condensed woff2 + events-current.json
 * - Emit ffw2025AssetMap.ts
 */
...
const CLASSIC_2025 = path.join(REPO_ROOT, 'DBS-FFW-classicsite/2025');
const ASSETS_ROOT = path.join(SPFX_ROOT, 'src/webparts/ffw2025/assets');
...
const MAP_OUT = path.join(ASSETS_ROOT, 'ffw2025AssetMap.ts');

const WEBP_QUALITY = 78;
const IMAGE_EXT = /\.(png|jpe?g)$/i;
const LIVE_STILLS = [
  'public/images/2025/logo.png',
  'public/images/2025/BG_for_Microsite.png',
...
const LIVE_GIF = 'public/images/2025/MICROSITE.gif';
...
function getImageMaxWidth(relPath) {
  ...
  if (p.includes('bg_for_microsite')) {
    return 1920;
  }
...
function classicToAbs(classicKey) {
  return path.join(CLASSIC_2025, classicKey);
}

function main() {
  console.log('Preparing Ffw2025 assets (live surface)…');
  ...
  for (const classicKey of LIVE_STILLS) {
    ...
    if (!fs.existsSync(src)) {
      console.warn(`Missing classic still (skip): ${src}`);
      continue;
    }
  ...
  const gifDest = path.join(IMG_OUT, '2025/MICROSITE.gif');
  copyFile(gifSrc, gifDest);
  imageEntries.push({ classicKey: LIVE_GIF, bundledPath: gifDest });

  copyFile(
    path.join(CLASSIC_2025, 'events-current.json'),
    path.join(DATA_OUT, 'events-current.json')
  );

  const fontsRoot = path.join(CLASSIC_2025, 'public/fonts/opensans-condensed');
  const fontFiles = [
  ...
  for (const font of fontFiles) {
    ...
    if (fs.existsSync(src)) {
      ...
      console.warn(`Missing font (skip): ${src}`);
    }
  }
...
`;
  ...
  console.log(`Images: ${imageEntries.length} (stills WebP q${WEBP_QUALITY} + GIF copy)`);
